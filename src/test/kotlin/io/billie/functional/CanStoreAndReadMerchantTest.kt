...
@ActiveProfiles("test")
class CanStoreAndReadMerchantTest {
    ...
    fun cannotStoreOrgWhenNameIsBlank() {
        mockMvc.perform(
            post("/merchants").contentType(APPLICATION_JSON).content(orgRequestJsonNameBlank())
        )
    ...
    fun cannotStoreOrgWhenNameIsMissing() {
        mockMvc.perform(
            post("/merchants").contentType(APPLICATION_JSON).content(orgRequestJsonNoName())
        )
            .andExpect(status().isBadRequest)
    }
    ...
    fun cannotStoreOrgWhenCountryCodeIsMissing() {
        mockMvc.perform(
            post("/merchants").contentType(APPLICATION_JSON).content(orgRequestJsonNoCountryCode())
        )
            .andExpect(status().isBadRequest)
    }
    ...
    fun cannotStoreOrgWhenCountryCodeIsBlank() {
        mockMvc.perform(
            post("/merchants").contentType(APPLICATION_JSON).content(orgRequestJsonCountryCodeBlank())
        )
            .andExpect(status().isBadRequest)
    }
    ...
    fun cannotStoreOrgWhenCountryCodeIsNotRecognised() {
        mockMvc.perform(
            post("/merchants").contentType(APPLICATION_JSON).content(orgRequestJsonCountryCodeIncorrect())
        )
            .andExpect(status().isBadRequest)
    }
    ...
    fun cannotStoreOrgWhenNoLegalEntityType() {
        mockMvc.perform(
            post("/merchants").contentType(APPLICATION_JSON).content(orgRequestJsonNoLegalEntityType())
        )
            .andExpect(status().isBadRequest)
    }
    ...
    fun cannotStoreOrgWhenNoContactDetails() {
        mockMvc.perform(
            post("/merchants").contentType(APPLICATION_JSON).content(orgRequestJsonNoContactDetails())
        )
            .andExpect(status().isBadRequest)
    }
    ...
    fun canStoreOrg() {
        val result = mockMvc.perform(
            post("/merchants").contentType(APPLICATION_JSON).content(orgRequestJson())
        )
        .andExpect(status().isOk)
        .andReturn()

        val response = mapper.readValue(result.response.contentAsString, CreatedMerchantViewModel::class.java)
